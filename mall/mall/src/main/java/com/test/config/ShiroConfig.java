...
public class ShiroConfig {
    ...
    public JdbcRealm createRealm(DruidDataSource ds) {
        ...
        //判断角色
        realm.setUserRolesQuery("select r.rname from role r left join user_role ur on r.rid=ur.rid left join user u on ur.uid=u.id where u.account=?");
       //判断权限
        realm.setPermissionsQuery("select rs.text from ress rs left join role_res rr on rs.id=rr.resid left join role r on rr.rid=r.rid where r.rname=?");

        return realm;
    }

    //调用createDS()方法,并将ds交由spring容器进行管理
    @Bean
